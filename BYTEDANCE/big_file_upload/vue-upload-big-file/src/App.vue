<template>
  <div id="app">
    ...
    <div>
      ...
      <div>总进度</div>
      <!-- 每个blob 进度    计算出来
      1. 每块blob上传  值percentage变化的， vue中 watch 监听
      2. 计算属性 computed -->
...
export default {
  ...
  computed: {
    uploadPercentage() {
      ...
      const loaded = this.data
        ...
        .reduce((acc, cur) => acc + cur)           // 已上传总文件大小
  ...
  methods: {
    ...
    resetData() {
      ...
      this.requestList = [];
      if (this.container.worker) {  // hash 计算过程
    ...
    async uploadChunks(uploadedList = []) {
      ...
      const requestList = this.data.map(({chunk, index, hash}) => {
